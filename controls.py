...
class Controls_Handler():
    ...
    def render(self, surface):
        self.draw_text(surface, "Input Settings for Player " + str(self.curr_block+1) , 20, pygame.Color((0,0,0)), settings.maxWidth / 4, 270/8)
    ...
    def display_controls(self,surface, controls):
        ...
        pygame.draw.rect(surface, color, (80 , settings.maxHeight/7 - 10 + (self.curr_index*30), 320, 20) )
        i = 0
        for control in controls:
            self.draw_text(surface, control + ' - ' + pygame.key.name(controls[control]),20, 
                            pygame.Color((0,0,0)), settings.maxWidth / 4, settings.maxHeight/7 + i)
            i += 30
        self.draw_text(surface, "Set Current Profile",20, pygame.Color((0,0,0)), settings.maxWidth / 4, settings.maxHeight/7 + i)
